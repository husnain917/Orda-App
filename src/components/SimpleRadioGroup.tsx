...
export default function SimpleRadioGroup(
  ...
  return (
    items.map(item => {
      return <ListItem onPress={() => setIndex(item.id)} style={ (item.id === items[items.length - 1].id) ? styles.itemNoUnderline : styles.itemUnderline}>
        <CheckBox checked={checkIndex(item.id)} color={colors.textInputColor} onPress={() => setIndex(item.id)}/>
        <Body>
          <Text style={{color: colors.darkGray, marginStart: 12, fontSize: 15 }}>{`${item.name} ${item.price ? `(${ absolutePrice ? '' : '+' }${currency}` + trimPrice(item.price) + ')' :  ''}`}</Text>
        </Body>
      </ListItem>
    })
  )
